...
//Generic query to fetch specified document data
export const query = ({ collection, orderBy = null, startAt = null, limit = 20, where }: any = {}) => {
  let ref: firebase.firestore.Query<firebase.firestore.DocumentData> = db.collection(collection); //Get refrence to the mentioned collection
  if (startAt !== null) ref = ref.startAt(startAt); //update if a starting condition is passed
  if (limit !== null) ref = ref.limit(limit); // document limit to be passed
  if (orderBy !== null) {
    const { field, op } = orderBy;
    ref = ref.orderBy(field, op); // sort the collection according to passed condition
  }
  if (where !== null) {
    const { field, op, value } = where;
    ref = ref.where(field, op, value); // quering the collection according to passed where condition
  }
  return ref;
};
...
// Get all the tasks for a specified user
